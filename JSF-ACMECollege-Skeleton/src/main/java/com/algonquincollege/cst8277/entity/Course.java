...
@NamedQuery(name = Course.ALL_COURSES_QUERY, 
    query = "SELECT c FROM Course c LEFT JOIN FETCH c.courseRegistrations")
@NamedQuery(name = Course.COURSE_BY_ID, 
    query = "SELECT c FROM Course c LEFT JOIN FETCH c.courseRegistrations WHERE c.id = :id")
// TODO C02 - Do we need a mapped super class?  If so, which one?
// Yes! Course extends PojoBase to inherit id, version, created, updated
// However, the primary key column is named "course_id" instead of "id"
...
public class Course extends PojoBase implements Serializable {
	...
	public static final String COURSE_BY_ID = "Course.findById";
	...
	// Cascade: MERGE is safe - we don't want to delete course registrations when updating a course
